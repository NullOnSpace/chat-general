...
pub async fn create_conversation(
    ...
    if req.participant_ids.is_empty() {
        return Err(AppError::Validation("At least one participant is required".into()));
    }
    
    let participants: Vec<UserId> = req.participant_ids
        .iter()
    ...
    if participants.len() == 1 {
        ...
        if !is_friend {
            return Err(AppError::Validation("Can only create direct conversation with friends".into()));
        }
...
pub async fn get_messages(
    ...
    let before = query.before.and_then(|b| chrono::DateTime::parse_from_rfc3339(&b).ok().map(|d| d.with_timezone(&chrono::Utc)));
    
    let messages = state.message_store.get_history(&conversation_id, before, limit).await?;
