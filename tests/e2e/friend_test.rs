...
async fn test_send_friend_request_success() {
    ...
    assert!(
        ...
        "Send friend request should succeed"
    );
}
...
async fn test_send_friend_request_already_friends() {
    ...
    assert!(
        !response.status().is_success(),
        "Friend request to already friend should fail"
    );
}
...
async fn test_send_friend_request_invalid_user() {
    ...
    assert!(
        !response.status().is_success(),
        "Friend request to invalid user should fail"
    );
}
...
async fn test_get_pending_requests() {
    ...
    assert!(
        response.status().is_success(),
        "Get pending requests should succeed"
    );
...
async fn test_accept_friend_request_invalid_id() {
    ...
    assert!(
        !response.status().is_success(),
        "Accept invalid request should fail"
    );
}
...
async fn test_reject_friend_request() {
    ...
    assert!(
        response.status().is_success(),
        "Reject friend request should succeed"
    );
}
...
async fn test_get_friends_list() {
    ...
    assert!(
        response.status().is_success(),
        "Get friends list should succeed"
    );

    let data: serde_json::Value = response.json().await.expect("Failed to parse response");
    let friends = data["friends"]
        .as_array()
        .expect("Should have friends array");
...
async fn test_friend_flow_complete() {
    ...
    let has_friend1 = friends1_data["friends"]
        .as_array()
        .unwrap()
        .iter()
        .any(|f| f["friend_id"].as_str() == Some(&user2.id));
    ...
    let has_friend2 = friends2_data["friends"]
        .as_array()
        .unwrap()
        .iter()
    ...
    let final_friends_response = client
        ...
        .await
        .expect("Failed to get friends");
    let final_friends_data: serde_json::Value = final_friends_response
        .json()
        .await
        .expect("Failed to parse");
    let still_has_friend = final_friends_data["friends"]
        .as_array()
        .unwrap()
        .iter()
        .any(|f| f["friend_id"].as_str() == Some(&user2.id));
