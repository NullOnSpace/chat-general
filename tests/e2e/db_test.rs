...
async fn test_db_friend_flow() {
    ...
    let requests = requests_data["requests"].as_array().expect("No requests array");
    ...
    assert!(accept_request.status().is_success(), "Accept request should succeed");
    ...
    let friends = friends_data["friends"].as_array().expect("No friends array");
...
async fn test_db_create_conversation_with_friend() {
    ...
    let req_id = reqs_data["requests"][0]["id"].as_str().expect("No request id");
    
    let accept = client
        .put(&format!("{}/api/v1/friends/requests/{}/accept", app.base_url(), req_id))
        .header("Authorization", format!("Bearer {}", user2_token))
        .send()
        .await
    ...
    assert!(create_conv.status().is_success(), "Create conversation with friend should succeed");
